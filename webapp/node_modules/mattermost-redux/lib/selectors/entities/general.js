...
function getFeatureFlagValue(state, key) {
    return getConfig(state)?.[`FeatureFlag${key}`];
}
function isCustomProfileAttributesEnabled(state) {
...
function isCompatibleWithJoinViewTeamPermissions(state) {
    ...
    return (0, helpers_1.isMinimumServerVersion)(version, 5, 10, 0) ||
        (version.indexOf('dev') !== -1 && (0, helpers_1.isMinimumServerVersion)(version, 5, 8, 0)) ||
        (version.match(/^5.8.\d.\d\d\d\d.*$/) !== null && (0, helpers_1.isMinimumServerVersion)(version, 5, 8, 0));
}
exports.canUploadFilesOnMobile = (0, create_selector_1.createSelector)('canUploadFilesOnMobile', getConfig, getLicense, (config, license) => {
...
exports.canDownloadFilesOnMobile = (0, create_selector_1.createSelector)('canDownloadFilesOnMobile', getConfig, getLicense, (config, license) => {
...
exports.getAutolinkedUrlSchemes = (0, create_selector_1.createSelector)('getAutolinkedUrlSchemes', getConfig, (config) => {
    if (!config.CustomUrlSchemes) {
        return constants_1.General.DEFAULT_AUTOLINKED_URL_SCHEMES;
    }
    return [
        ...constants_1.General.DEFAULT_AUTOLINKED_URL_SCHEMES,
        ...config.CustomUrlSchemes.split(','),
    ];
});
exports.getManagedResourcePaths = (0, create_selector_1.createSelector)('getManagedResourcePaths', getConfig, (config) => {
...
function getFirstAdminVisitMarketplaceStatus(state) {
    return state.entities.general.firstAdminVisitMarketplaceStatus;
}
function getFirstAdminSetupComplete(state) {
    return state.entities.general.firstAdminCompleteSetup;
}
function isPerformanceDebuggingEnabled(state) {
    return state.entities.general.config.EnableClientPerformanceDebugging === 'true';
}
exports.isMarketplaceEnabled = (0, create_selector_1.createSelector)('isMarketplaceEnabled', getConfig, (config) => {
    return config.PluginsEnabled === 'true' && config.EnableMarketplace === 'true';
});
exports.getUsersStatusAndProfileFetchingPollInterval = (0, create_selector_1.createSelector)('getUsersStatusAndProfileFetchingPollInterval', getConfig, (config) => {
...
function developerModeEnabled(state) {
