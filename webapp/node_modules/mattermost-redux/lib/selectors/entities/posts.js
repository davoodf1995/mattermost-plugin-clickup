...
exports.isPersistentNotificationsEnabled = exports.makeIsPostCommentMention = exports.isPostIdSending = exports.isPostsChunkIncludingUnreadsPosts = exports.getMostRecentPostIdInChannel = exports.getSearchResults = exports.getPostsInCurrentChannel = exports.getLatestReplyablePostId = void 0;
...
exports.getLatestInteractablePostId = getLatestInteractablePostId;
exports.getLatestPostToEdit = getLatestPostToEdit;
...
exports.getOldestPostTimeInChannel = getOldestPostTimeInChannel;
...
exports.getLimitedViews = getLimitedViews;
exports.isPostPriorityEnabled = isPostPriorityEnabled;
exports.isPostAcknowledgementsEnabled = isPostAcknowledgementsEnabled;
exports.getAllowPersistentNotifications = getAllowPersistentNotifications;
exports.getPersistentNotificationMaxRecipients = getPersistentNotificationMaxRecipients;
exports.getPersistentNotificationIntervalMinutes = getPersistentNotificationIntervalMinutes;
exports.getAllowPersistentNotificationsForGuests = getAllowPersistentNotificationsForGuests;
exports.getPostAcknowledgements = getPostAcknowledgements;
exports.makeGetPostAcknowledgementsWithProfiles = makeGetPostAcknowledgementsWithProfiles;
exports.getTeamIdFromPost = getTeamIdFromPost;
...
function isPostFlagged(state, postId) {
    return (0, preferences_1.getBool)(state, constants_1.Preferences.CATEGORY_FLAGGED_POST, postId);
}
...
function getPostsInThreadOrdered(state, rootId) {
    ...
    const sortedPosts = threadPosts.sort(post_utils_1.comparePosts);
...
function makeGetReactionsForPost() {
    return (0, create_selector_1.createSelector)('makeGetReactionsForPost', getReactionsForPosts, (state, postId) => postId, (reactions, postId) => {
        if (reactions[postId]) {
...
function getHasReactions(state, postId) {
...
function makeGetPostIdsForThread() {
    ...
    return (0, helpers_1.createIdsSelector)('makeGetPostIdsForThread', (state, rootId) => getPostsForThread(state, rootId), (posts) => {
        return posts.map((post) => post.id);
    });
}
function makeGetPostsChunkAroundPost() {
    return (0, helpers_1.createIdsSelector)('makeGetPostsChunkAroundPost', (state, postId, channelId) => state.entities.posts.postsInChannel[channelId], (state, postId) => postId, (postsForChannel, postId) => {
        if (!postsForChannel) {
            return null;
        }
        let postChunk;
...
function isPostInteractable(post) {
    return post &&
        !post.delete_at &&
...
function getLatestPostToEdit(state, channelId, rootId = '') {
    const postsIds = rootId ? getPostsInThreadOrdered(state, rootId) : getPostIdsInChannel(state, channelId);
    if (!postsIds) {
        return '';
    }
...
const getLatestReplyablePostId = (state) => getLatestInteractablePostId(state, (0, common_1.getCurrentChannelId)(state));
...
exports.getPostsInCurrentChannel = (0, create_selector_1.createSelector)('getPostsInCurrentChannel', getAllPosts, getPostIdsInCurrentChannel, common_1.getCurrentUser, preferences_1.shouldShowJoinLeaveMessages, (allPosts, postIds, currentUser, showJoinLeave) => {
    if (!postIds) {
        return null;
    }
    const posts = [];
    for (let i = 0; i < postIds.length; i++) {
        ...
        if (!post || (0, post_utils_1.shouldFilterJoinLeavePost)(post, showJoinLeave, currentUser ? currentUser.username : '')) {
            continue;
        }
        posts.push(post);
    }
...
function makeGetPostsForThread() {
    return (0, helpers_1.createIdsSelector)('makeGetPostsForThread', getAllPosts, common_1.getCurrentUser, (state, rootId) => state.entities.posts.postsInThread[rootId], (state, rootId) => state.entities.posts.posts[rootId], preferences_1.shouldShowJoinLeaveMessages, (posts, currentUser, postsForThread, rootPost, showJoinLeave) => {
        const thread = [];
        if (rootPost) {
        ...
        if (postsForThread && Array.isArray(postsForThread) && postsForThread.length > 0) {
            for (const postId of postsForThread) {
                ...
                const skip = (0, post_utils_1.shouldFilterJoinLeavePost)(post, showJoinLeave, currentUser ? currentUser.username : '');
                if (!skip) {
                    thread.push(post);
                }
            }
...
function makeGetProfilesForThread() {
    const getPostsForThread = makeGetPostsForThread();
    return (0, create_selector_1.createSelector)('makeGetProfilesForThread', users_1.getUsers, users_1.getCurrentUserId, getPostsForThread, users_1.getUserStatuses, (allUsers, currentUserId, posts, userStatuses) => {
        const profileIds = posts.map((post) => post.user_id).filter(Boolean);
        const uniqueIds = [...new Set(profileIds)];
        return uniqueIds.reduce((acc, id) => {
            const profile = userStatuses ? { ...allUsers[id], status: userStatuses[id] } : { ...allUsers[id] };
...
function makeGetCommentCountForPost() {
    return (0, create_selector_1.createSelector)('makeGetCommentCountForPost', getAllPosts, (state, post) => state.entities.posts.postsInThread[post ? post.root_id || post.id : ''] || null, (state, post) => post, (posts, postsForThread, post) => {
        if (!post) {
            return 0;
        }
        if (!postsForThread) {
        ...
        postsForThread.forEach((id) => {
            const post = posts[id];
            if (post && post.state !== constants_1.Posts.POST_DELETED && !(0, post_utils_1.isPostEphemeral)(post)) {
                count += 1;
            }
        });
        return count;
...
exports.getSearchResults = (0, create_selector_1.createSelector)('getSearchResults', getAllPosts, (state) => state.entities.search.results, (posts, postIds) => {
    if (!postIds) {
        return [];
    }
...
function makeGetMessageInHistoryItem(type) {
    return (0, create_selector_1.createSelector)('makeGetMessageInHistoryItem', (state) => state.entities.posts.messagesHistory, (messagesHistory) => {
...
function makeGetPostsForIds() {
    return (0, helpers_1.createIdsSelector)('makeGetPostsForIds', getAllPosts, (state, postIds) => postIds, (allPosts, postIds) => {
        if (!postIds) {
            return [];
        }
...
exports.getMostRecentPostIdInChannel = (0, create_selector_1.createSelector)('getMostRecentPostIdInChannel', getAllPosts, (state, channelId) => getPostIdsInChannel(state, channelId), preferences_1.shouldShowJoinLeaveMessages, (posts, postIdsInChannel, allowSystemMessages) => {
    if (!postIdsInChannel) {
        return '';
    }
...
function getOldestPostTimeInChannel(state, channelId) {
    const postsForChannel = state.entities.posts.postsInChannel[channelId];
    if (!postsForChannel) {
        return 0;
    }
    const allPosts = getAllPosts(state);
    const oldestPostTime = postsForChannel.reduce((acc, postBlock) => {
...
exports.isPostsChunkIncludingUnreadsPosts = isPostsChunkIncludingUnreadsPosts;
const isPostIdSending = (state, postId) => {
...
const makeIsPostCommentMention = () => {
    return (0, create_selector_1.createSelector)('makeIsPostCommentMention', getAllPosts, getPostsInThread, common_1.getCurrentUser, getPost, (allPosts, postsInThread, currentUser, post) => {
        if (!post) {
            return false;
        }
        ...
        if (currentUser) {
            ...
            isCommentMention = (0, post_utils_1.isPostCommentMention)({ post, currentUser, threadRepliedToByCurrentUser, rootPost });
        }
        return isCommentMention;
    });
};
exports.makeIsPostCommentMention = makeIsPostCommentMention;
function getLimitedViews(state) {
    return state.entities.posts.limitedViews;
}
function isPostPriorityEnabled(state) {
    return (0, general_1.getConfig)(state).PostPriority === 'true';
}
function isPostAcknowledgementsEnabled(state) {
    return (isPostPriorityEnabled(state) &&
        (0, general_1.getConfig)(state).PostAcknowledgements === 'true');
}
function getAllowPersistentNotifications(state) {
    return (isPostPriorityEnabled(state) &&
        (0, general_1.getConfig)(state).AllowPersistentNotifications === 'true');
}
function getPersistentNotificationMaxRecipients(state) {
    return (0, general_1.getConfig)(state).PersistentNotificationMaxRecipients;
}
function getPersistentNotificationIntervalMinutes(state) {
    return (0, general_1.getConfig)(state).PersistentNotificationIntervalMinutes;
}
function getAllowPersistentNotificationsForGuests(state) {
    return (isPostPriorityEnabled(state) &&
        (0, general_1.getConfig)(state).AllowPersistentNotificationsForGuests === 'true');
}
function getPostAcknowledgements(state, postId) {
    return state.entities.posts.acknowledgements[postId];
}
exports.isPersistentNotificationsEnabled = (0, create_selector_1.createSelector)('getPersistentNotificationsEnabled', common_1.getCurrentUser, getAllowPersistentNotifications, getAllowPersistentNotificationsForGuests, (user, forAll, forGuests) => ((0, user_utils_1.isGuest)(user.roles) ? (forAll && forGuests) : forAll));
function makeGetPostAcknowledgementsWithProfiles() {
    return (0, create_selector_1.createSelector)('makeGetPostAcknowledgementsWithProfiles', users_1.getUsers, getPostAcknowledgements, (users, acknowledgements) => {
        if (!acknowledgements) {
            return [];
        }
...
function getTeamIdFromPost(state, post) {
    ...
    if (channel.type === constants_1.General.DM_CHANNEL || channel.type === constants_1.General.GM_CHANNEL) {
        return (0, teams_1.getCurrentTeamId)(state);
    }
