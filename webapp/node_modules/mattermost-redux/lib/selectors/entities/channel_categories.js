...
exports.getNonManagedCategoryOrderForTeam = getNonManagedCategoryOrderForTeam;
exports.makeGetNonManagedCategoriesForTeam = makeGetNonManagedCategoriesForTeam;
...
exports.makeGetChannelIdsForCategory = makeGetChannelIdsForCategory;
...
function getNonManagedCategoryOrderForTeam(state, teamId) {
    return state.entities.channelCategories.orderByTeam[teamId];
}
function makeGetNonManagedCategoriesForTeam() {
    return (0, create_selector_1.createSelector)('makeGetNonManagedCategoriesForTeam', getNonManagedCategoryOrderForTeam, (state) => state.entities.channelCategories.byId, (categoryIds, categoriesById) => {
        if (!categoryIds) {
            return [];
        }
...
function makeFilterArchivedChannels() {
    return (0, create_selector_1.createSelector)('makeFilterArchivedChannels', (state, channels) => channels, channels_1.getCurrentChannelId, (channels, currentChannelId) => {
...
function makeFilterAutoclosedDMs() {
    return (0, create_selector_1.createSelector)('makeFilterAutoclosedDMs', (state, channels) => channels, (state, channels, categoryType) => categoryType, channels_1.getCurrentChannelId, (state) => state.entities.users.profiles, users_1.getCurrentUserId, channels_1.getMyChannelMemberships, channels_1.getChannelMessageCounts, (state) => (0, preferences_1.getVisibleDmGmLimit)(state), preferences_1.getMyPreferences, preferences_1.isCollapsedThreadsEnabled, (channels, categoryType, currentChannelId, profiles, currentUserId, myMembers, messageCounts, limitPref, myPreferences, collapsedThreads) => {
        if (categoryType !== channel_categories_2.CategoryTypes.DIRECT_MESSAGES) {
            // Only autoclose DMs that haven't been assigned to a category
            return channels;
        }
        const getTimestampFromPrefs = (category, name) => {
            const pref = myPreferences[(0, preference_utils_1.getPreferenceKey)(category, name)];
            return parseInt(pref ? pref.value : '0', 10);
        };
        const getLastViewedAt = (channel) => {
            ...
            return Math.max(myMembers[channel.id]?.last_viewed_at, getTimestampFromPrefs(constants_1.Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME, channel.id), getTimestampFromPrefs(constants_1.Preferences.CATEGORY_CHANNEL_OPEN_TIME, channel.id));
        };
        let unreadCount = 0;
...
function makeFilterManuallyClosedDMs() {
    return (0, create_selector_1.createSelector)('makeFilterManuallyClosedDMs', (state, channels) => channels, preferences_1.getMyPreferences, channels_1.getCurrentChannelId, users_1.getCurrentUserId, channels_1.getMyChannelMemberships, channels_1.getChannelMessageCounts, preferences_1.isCollapsedThreadsEnabled, (channels, myPreferences, currentChannelId, currentUserId, myMembers, messageCounts, collapsedThreads) => {
        const filtered = channels.filter((channel) => {
            ...
            if (channel.type === constants_1.General.DM_CHANNEL) {
                const teammateId = (0, channel_utils_1.getUserIdFromChannelName)(currentUserId, channel.name);
                preference = myPreferences[(0, preference_utils_1.getPreferenceKey)(constants_1.Preferences.CATEGORY_DIRECT_CHANNEL_SHOW, teammateId)];
            }
            else {
                preference = myPreferences[(0, preference_utils_1.getPreferenceKey)(constants_1.Preferences.CATEGORY_GROUP_CHANNEL_SHOW, channel.id)];
            }
...
function makeCompareChannels(getDisplayName, locale, myMembers) {
    return (a, b) => {
        ...
        const aMuted = (0, channel_utils_1.isChannelMuted)(myMembers[a.id]);
        const bMuted = (0, channel_utils_1.isChannelMuted)(myMembers[b.id]);
        if (aMuted && !bMuted) {
            return 1;
        }
...
function makeSortChannelsByName() {
    return (0, create_selector_1.createSelector)('makeSortChannelsByName', (state, channels) => channels, (state) => (0, i18n_1.getCurrentUserLocale)(state), channels_1.getMyChannelMemberships, (channels, locale, myMembers) => {
        const getDisplayName = (channel) => channel.display_name;
        return [...channels].sort(makeCompareChannels(getDisplayName, locale, myMembers));
    });
}
function makeSortChannelsByNameWithDMs() {
    return (0, create_selector_1.createSelector)('makeSortChannelsByNameWithDMs', (state, channels) => channels, users_1.getCurrentUserId, (state) => state.entities.users.profiles, preferences_1.getTeammateNameDisplaySetting, (state) => (0, i18n_1.getCurrentUserLocale)(state), channels_1.getMyChannelMemberships, (channels, currentUserId, profiles, teammateNameDisplay, locale, myMembers) => {
        const cachedNames = {};
        const getDisplayName = (channel) => {
            ...
            if (channel.type === constants_1.General.DM_CHANNEL) {
                const teammateId = (0, channel_utils_1.getUserIdFromChannelName)(currentUserId, channel.name);
                const teammate = profiles[teammateId];
                displayName = (0, user_utils_1.displayUsername)(teammate, teammateNameDisplay, false);
            }
                ...
                for (const username of usernames) {
                    ...
                    userDisplayNames.push((0, user_utils_1.displayUsername)(user, teammateNameDisplay, false));
                }
...
function makeSortChannelsByRecency() {
    return (0, create_selector_1.createSelector)('makeSortChannelsByRecency', (_state, channels) => channels, preferences_1.isCollapsedThreadsEnabled, (channels, crtEnabled) => {
...
function makeSortChannels() {
    ...
    return (state, originalChannels, category) => {
        ...
        if (category.sorting === channel_categories_1.CategorySorting.Recency) {
            channels = sortChannelsByRecency(state, channels);
        }
        else if (category.sorting === channel_categories_1.CategorySorting.Alphabetical || category.sorting === channel_categories_1.CategorySorting.Default) {
            if (channels.some((channel) => channel.type === constants_1.General.DM_CHANNEL || channel.type === constants_1.General.GM_CHANNEL)) {
...
function makeGetChannelIdsForCategory() {
    const getChannels = (0, channels_1.makeGetChannelsForIds)();
    ...
    return (state, category) => {
        ...
        if ((0, shallow_equals_1.default)(filteredChannelIds, lastChannelIds)) {
...
function makeGetChannelsByCategory() {
    ...
    let getChannels;
    let filterAndSortChannels;
    let lastCategories = [];
    let lastChannelsByCategory = {};
    return (state, teamId) => {
        ...
        if (categories !== lastCategories) {
            ...
            for (const category of categories) {
                getChannels[category.id] = (0, channels_1.makeGetChannelsForIds)();
        ...
        for (const category of categories) {
            ...
            allEquals = allEquals && (0, shallow_equals_1.default)(channelsByCategory[category.id], lastChannelsByCategory[category.id]);
        }
...
function isUnreadChannel(channelId, messageCounts, members, crtEnabled) {
    const unreadCount = (0, channel_utils_1.calculateUnreadCount)(messageCounts[channelId], members[channelId], crtEnabled);
...
function isChannelInManagedCategory(state, channelId) {
    if (!areManagedCategoriesEnabled(state)) {
        return false;
    }
...
function getChannelManagedCategoryName(state, channelId) {
    if (!areManagedCategoriesEnabled(state)) {
        return undefined;
    }
    const channel = (0, channels_1.getChannel)(state, channelId);
...
function makeGetCategoriesForTeam() {
    ...
    return (0, create_selector_1.createSelector)('makeGetCategoriesForTeam', getNonManagedCategoriesForTeam, getManagedCategoriesForTeam, (nonManagedCategories, managedCategories) => {
...
function makeGetManagedCategoriesForTeam() {
    return (0, create_selector_1.createSelector)('makeGetManagedCategoriesForTeam', (state, teamId) => getManagedCategoryMappings(state, teamId), (state) => (0, users_1.getCurrentUserId)(state), (_, teamId) => teamId, (mappings, currentUserId, teamId) => {
        ...
        const channelsByCategory = Object.create(null);
        ...
        const sortedNames = Object.keys(channelsByCategory).sort((a, b) => (a.localeCompare(b, undefined, { numeric: true })));
        return sortedNames.map((name) => {
            ...
            return {
                ...
                team_id: teamId,
                type: channel_categories_2.CategoryTypes.MANAGED,
                display_name: name,
                sorting: channel_categories_1.CategorySorting.Alphabetical,
