...
const message_counts_1 = __importDefault(require("./channels/message_counts"));
...
function channels(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_CHANNEL: {
            const channel = toClientChannel(action.data);
            if (state[channel.id] && channel.type === constants_1.General.DM_CHANNEL) {
        ...
        case action_types_1.AdminTypes.RECEIVED_DATA_RETENTION_CUSTOM_POLICY_CHANNELS: {
            const channels = action.data.channels.map(toClientChannel);
            if (channels.length === 0) {
                return state;
            }
            return {
                ...state,
                ...(0, channel_utils_1.channelListToMap)(channels),
            };
        }
        ...
        case action_types_1.SchemeTypes.RECEIVED_SCHEME_CHANNELS: {
            const channels = action.data.map(toClientChannel);
            if (channels.length === 0) {
                return state;
            }
            const nextState = { ...state };
        ...
        case action_types_1.AdminTypes.REMOVE_DATA_RETENTION_CUSTOM_POLICY_CHANNELS_SUCCESS: {
...
function toClientChannel(serverChannel) {
...
function myMembers(state = {}, action) {
    switch (action.type) {
            ...
            return {
                ...
                [channelId]: {
                    ...
                    notify_props: {
                        ...
                        mark_unread: muted ? channels_1.MarkUnread.MENTION : channels_1.MarkUnread.ALL,
                    },
                },
            };
        }
        case action_types_1.ChannelTypes.INCREMENT_UNREAD_MSG_COUNT: {
            const { channelId, amount, amountRoot, onlyMentions, fetchedChannelMember, } = action.data;
        ...
        case action_types_1.ChannelTypes.INCREMENT_UNREAD_MENTION_COUNT: {
            const { channelId, amount, amountRoot, amountUrgent, fetchedChannelMember, } = action.data;
            if (amount === 0 && amountRoot === 0) {
                return state;
            }
            const member = state[channelId];
...
function receiveChannelMember(state, received) {
    const existingChannelMember = state[received.channel_id];
    let updatedChannelMember = { ...received };
    if ((0, isEqual_1.default)(existingChannelMember, updatedChannelMember)) {
        return state;
    }
...
function stats(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_CHANNEL_STATS: {
            ...
            if ((0, isEqual_1.default)(state[stat.channel_id], stat)) {
                return state;
            }
            return {
                ...state,
        ...
        case action_types_1.ChannelTypes.INCREMENT_FILE_COUNT: {
            const nextState = { ...state };
            const id = action.id;
            const nextStat = nextState[id];
            if (nextStat) {
...
function channelsMemberCount(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_CHANNELS_MEMBER_COUNT: {
...
function channelMemberCountsByGroup(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_CHANNEL_MEMBER_COUNTS_BY_GROUP: {
            ...
            const memberCountsByGroup = {};
            memberCounts.forEach((channelMemberCount) => {
        ...
        case action_types_1.ChannelTypes.RECEIVED_CHANNEL_MEMBER_COUNTS_FROM_GROUPS_LIST: {
            const memberCountsByGroup = {};
...
function roles(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_MY_CHANNEL_MEMBER: {
            ...
            const newRoles = (0, channel_utils_1.splitRoles)(channelMember.roles);
            // If roles didn't change no need to update state
            if ((0, isEqual_1.default)(oldRoles, newRoles)) {
                return state;
            }
            return {
                ...state,
        ...
        case action_types_1.ChannelTypes.RECEIVED_MY_CHANNEL_MEMBERS: {
            ...
            for (const cm of action.data) {
                ...
                const newRoles = (0, channel_utils_1.splitRoles)(cm.roles);
                // If roles didn't change no need to update state
                if (!(0, isEqual_1.default)(oldRoles, newRoles)) {
                    nextState[cm.channel_id] = (0, channel_utils_1.splitRoles)(cm.roles);
                    stateChanged = true;
                }
            }
            if (stateChanged) {
...
function restrictedDMs(state = {}, action) {
    switch (action.type) {
        case action_types_1.ChannelTypes.RECEIVED_IS_DM_RESTRICTED: {
            return {
                ...state,
                [action.data.channelId]: action.data.isRestricted,
            };
        }
        case action_types_1.ChannelTypes.RESTRICTED_DMS_TEAMS_CHANGED:
        case action_types_1.UserTypes.LOGOUT_SUCCESS:
            return {};
        default:
            return state;
...
exports.default = (0, redux_1.combineReducers)({
    ...
    messageCounts: message_counts_1.default,
