...
function unfavoriteChannel(channelId) {
    return async (dispatch, getState) => {
        ...
        return dispatch((0, channel_categories_3.addChannelToCategory)(category.id, channel.id));
    };
}
function updateChannelScheme(channelId, schemeId) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            await client_1.Client4.updateChannelScheme(channelId, schemeId);
...
function updateChannelMemberSchemeRoles(channelId, userId, isSchemeUser, isSchemeAdmin) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            await client_1.Client4.updateChannelMemberSchemeRoles(channelId, userId, isSchemeUser, isSchemeAdmin);
...
function membersMinusGroupMembers(channelID, groupIDs, page = 0, perPage = constants_1.General.PROFILE_CHUNK_SIZE) {
    return (0, helpers_1.bindClientFunc)({
...
function getChannelModerations(channelId) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            const moderations = await client_1.Client4.getChannelModerations(channelId);
...
function patchChannelModerations(channelId, patch) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            const moderations = await client_1.Client4.patchChannelModerations(channelId, patch);
            return { channelId, moderations };
        },
        onSuccess: action_types_1.ChannelTypes.RECEIVED_CHANNEL_MODERATIONS,
    });
}
function getChannelMemberCountsByGroup(channelId) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            const channelMemberCountsByGroup = await client_1.Client4.getChannelMemberCountsByGroup(channelId, true);
...
function fetchMissingChannels(channelIDs, asContentReviewer = false) {
    return async (dispatch, getState, { loaders }) => {
        if (!loaders.missingChannelLoader) {
            loaders.missingChannelLoader = new data_loader_1.DelayedDataLoader({
        ...
        const state = getState();
        const missingChannelIDs = channelIDs.filter((channelId) => !(0, channels_2.getChannel)(state, channelId));
        if (missingChannelIDs.length > 0) {
            const loader = loaders.missingChannelLoader;
...
function fetchIsRestrictedDM(channelId) {
    return (0, helpers_1.bindClientFunc)({
        clientFunc: async () => {
            const teams = (await client_1.Client4.getGroupMessageMembersCommonTeams(channelId)).data;
            return { channelId, isRestricted: teams.length === 0 };
        },
        onSuccess: action_types_1.ChannelTypes.RECEIVED_IS_DM_RESTRICTED,
    });
}
