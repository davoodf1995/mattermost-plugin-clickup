...
function acknowledgePost(postId) {
    return async (dispatch, getState) => {
        ...
        dispatch({
            type: action_types_1.PostTypes.CREATE_ACK_POST_SUCCESS,
            data,
        });
        return { data };
    };
...
function unacknowledgePost(postId) {
    return async (dispatch, getState) => {
        const userId = (0, users_2.getCurrentUserId)(getState());
        try {
            await client_1.Client4.unacknowledgePost(postId, userId);
        }
        catch (error) {
            (0, helpers_1.forceLogoutIfNecessary)(error, dispatch, getState);
            dispatch((0, errors_1.logError)(error));
        ...
        dispatch({
            type: action_types_1.PostTypes.DELETE_ACK_POST_SUCCESS,
            data,
        });
        return { data };
    };
}
function restorePostVersion(postId, restoreVersionId, connectionId) {
    return async (dispatch, getState) => {
        try {
            await client_1.Client4.restorePostVersion(postId, restoreVersionId, connectionId);
        }
        catch (error) {
